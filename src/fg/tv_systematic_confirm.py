...
VALUES = ['hasusbport', 'ecorating', 'family', 'hdmiport', 'screensizerange',
          'pricerange']

@register("d2t.tv_systematic_confirm")
class TVSystematicConfirm(PlumObject):
    ...
    def make_samples(self):
        ...
        for setting in settings:
            for p in product(*([['']] + [['', "_dontcare"]] * len(setting[:-1]))):
                if 'hasusbport_false' in setting:
                    idx = setting.index('hasusbport_false')
                    if p[idx] == "_dontcare":
                        continue
                mr = {"da": "?confirm", "fields": {}}
